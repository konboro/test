...
import { normalisePhone, segmentCount, sendSms } from './send';
...
describe('normalisePhone', () => {
  ...
  it('refuses anything it cannot vouch for', () => {
    // Skipping the send is cheaper than billing a segment nobody receives.
...
describe('segmentCount', () => {
  ...
  it('counts Greek text against the 70/67 UCS-2 limits', () => {
    // This is the cost trap: a Greek reminder over 70 characters is two segments.
...
describe('sendSms transport', () => {
  it('posts a form-encoded message to the account it authenticates as', async () => {
    withCredentials();
    const calls = mockFetch({ body: { sid: 'SM123' } });

    const result = await sendSms({ phone: '6971234567', message: 'Υπενθύμιση' });

    expect(result).toEqual({ ok: true, messageId: 'SM123' });
    ...
    expect(headers.Authorization).toBe(`Basic ${btoa('ACtest123:secret-token')}`);

    const form = init.body as URLSearchParams;
    expect(form.get('To')).toBe('+306971234567');
    expect(form.get('From')).toBe('lefta');
    expect(form.get('Body')).toBe('Υπενθύμιση');
    // Stale reminders should expire in the queue, not arrive days later.
    expect(form.get('ValidityPeriod')).toBe('36000');
  });
  ...
  it('sends from the configured brand when one is set', async () => {
    ...
    expect((firstCall(calls).init.body as URLSearchParams).get('From')).toBe('Penny');
  });
  ...
  it('surfaces the provider’s reason for a rejection', async () => {
    ...
    mockFetch({
      ok: false,
      status: 400,
      body: { code: 21612, message: 'The To phone number is not currently reachable' },
    });
    ...
    expect(result.error).toContain('not currently reachable');
  });
